...
export function render() {
  ...
  document.getElementById('footer').innerHTML = `
    <a href="/new-park" class="lzr-button lzr-outlined lzr-primary" style="margin: auto">Nouveau jardin</a>
  `;
}
...
function getParksDom() {
  ...
  for (let park of user.parks) {
    ...
    <button class="lzr-button lzr-square lzr-outlined" onclick="onEditParkClick(${park.id})">${getSvgIcon('ellipsis-vertical')}</button>
      <a href="/park?id=${park.id}" class="lzr-button lzr-solid lzr-primary" style="width: 100%;">${park.name}</a>
    </div>
    `;
  }
